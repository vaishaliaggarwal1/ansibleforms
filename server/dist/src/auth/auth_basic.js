...
passport.use('basic', new basicStrategy(async (username, password, done) => {
  ...
  try {
    ...
    var user = await User.authenticate(username, password).then(async result => {
      var user = {};
      // user found in db
...
passport.use('ldap', new basicStrategy(async (username, password, done) => {
  // authentication against database first
  try {
    var ldapConfig = await Ldap.find();
    var user = await User.checkLdap(username, password).then(async result => {
