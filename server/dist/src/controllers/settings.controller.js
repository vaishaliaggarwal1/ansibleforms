...
exports.find = function (req, res) {
  Settings.find().then(settings => {
    ...
    res.json(new RestResult("error", "Failed to find settings", null, Helpers.getError(err)));
  });
};
exports.mailcheck = function (req, res) {
  Settings.mailcheck(new Settings(req.body), req.body.to).then(messageid => {
...
exports.update = function (req, res) {
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    Settings.update(new Settings(req.body)).then(() => {
...
exports.importFormsFileFromYaml = function (req, res) {
  Settings.importFormsFileFromYaml().then(() => {
    res.json(new RestResult("success", "Forms imported", null, ""));
  }).catch(err => {
    res.json(new RestResult("error", "Failed to import forms.yaml", null, Helpers.getError(err)));
  });
};