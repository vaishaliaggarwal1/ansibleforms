...
exports.refresh = function (req, res) {
  ...
  if (!refreshtoken) {
    ...
    if (jwtPayload && jwtPayload.user && jwtPayload.refresh) {
      ...
      User.checkToken(username, username_type, refreshtoken).then(result => {
        ...
        if (new Date(jwtPayload.exp * 1000) > new Date()) {
          ...
          User.storeToken(username, username_type, refreshtoken).then(() => {
          ...
          User.deleteToken(username, username_type, refreshtoken).then(() => {
