...
exports.find = function (req, res) {
  if (req.query.name) {
    Group.findByName(req.query.name).then(groups => {
      if (groups.length > 0) {
      ...
      res.json(new RestResult("error", "failed to find group", null, err.toString()));
    });
  } else {
    Group.findAll().then(groups => {
      res.json(new RestResult("success", "groups found", groups, ""));
    }).catch(err => {
      res.json(new RestResult("error", "failed to find groups", null, err.toString()));
    });
  }
};
exports.create = function (req, res) {
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    Group.create(new_group).then(group => {
      res.json(new RestResult("success", "group added", group, ""));
    }).catch(err => {
      res.json(new RestResult("error", "failed to create group", null, err.toString()));
    });
  }
};
...
exports.update = function (req, res) {
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    Group.update(new Group(req.body), req.params.id).then(() => {
      res.json(new RestResult("success", "group updated", null, ""));
    }).catch(err => {
      res.json(new RestResult("error", "failed to update group", null, err.toString()));
    });
  }
};
