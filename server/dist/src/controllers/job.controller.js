...
exports.abortJob = function (req, res) {
  ...
  Job.abort(jobid).then(job => {
    res.json(new RestResult("success", "job aborted", null, ""));
  }).catch(err => {
    res.json(new RestResult("error", "failed to abort job", null, err.toString()));
  });
};
exports.getJob = function (req, res) {
  var user = req.user.user;
  Job.findById(user, req.params.id, req.query.text == "true", true).then(job => {
    if (job.length > 0) {
    ...
    res.json(new RestResult("error", "failed to find job", null, err.toString()));
  });
};
exports.findAllJobs = function (req, res) {
  var user = req?.user?.user || {};
  var records = req.query.records || 500;
  Job.findAll(user, records).then(jobs => {
    res.json(new RestResult("success", "jobs found", jobs, ""));
  }).catch(err => {
    res.json(new RestResult("error", "failed to find jobs", null, err.toString()));
  });
};
exports.findApprovals = function (req, res) {
  var user = req.user.user;
  Job.findApprovals(user).then(count => {
    if (count) {
...
exports.deleteJob = function (req, res) {
  Job.delete(req.params.id).then(job => {
