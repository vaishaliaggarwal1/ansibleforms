...
exports.find = function (req, res) {
  if (req.query.name) {
    Credential.findByName2(req.query.name).then(credential => {
      res.json(new RestResult("success", "credentials found", credential, ""));
    }).catch(err => {
      res.json(new RestResult("error", "failed to find credentials", null, err.toString()));
    });
  } else {
    Credential.findAll().then(credentials => {
...
exports.create = function (req, res) {
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    Credential.create(new_credential).then(credential => {
      res.json(new RestResult("success", "credential added", credential, ""));
    }).catch(err => {
      res.json(new RestResult("error", "failed to create credential", null, err.toString()));
    });
  }
};
exports.findById = function (req, res) {
  Credential.findById(req.params.id).then(credential => {
...
exports.update = function (req, res) {
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    Credential.update(new Credential(req.body), req.params.id).then(() => {
      res.json(new RestResult("success", "credential updated", null, ""));
    }).catch(err => {
      res.json(new RestResult("error", "failed to update credential", null, err.toString()));
    });
  }
};
exports.delete = function (req, res) {
  Credential.delete(req.params.id).then(() => {
    res.json(new RestResult("success", "credential deleted", null, ""));
  }).catch(err => {
    res.json(new RestResult("error", "failed to delete credential", null, err.toString()));
  });
};
exports.testDb = function (req, res) {
  Credential.findById(req.params.id).then(cred => {
    ...
    res.json(new RestResult("success", "Database connection ok", null, ""));
  }).catch(err => {
