...
exports.findAllOr1 = function (req, res) {
  if (req.query.username) {
    User.findByUsername(req.query.username).then(user => {
    ...
    User.findAll().then(users => {
      ...
      res.json(new RestResult("error", "failed to find users", null, err.toString()));
    });
  }
};
exports.create = function (req, res) {
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    User.create(new_user).then(user => {
      ...
      res.json(new RestResult("error", "failed to create user", null, err.toString()));
    });
  }
};
...
exports.findByToken = function (req, res) {
  User.findById(req.user.user.username).then(user => {
    if (user.length > 0) {
...
exports.update = function (req, res) {
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    User.update(new User(req.body), req.params.id).then(() => {
      res.json(new RestResult("success", "user updated", null, ""));
    }).catch(err => {
      res.json(new RestResult("error", "failed to update user", null, err.toString()));
    });
  }
};
exports.changePassword = function (req, res) {
  if (req.user.user.type == "local" && req.user.user.id) {
    ...
    if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
      ...
      User.update(new User(req.body), req.user.user.id).then(user => {
        res.json(new RestResult("success", "password changed", null, ""));
      }).catch(err => {
        res.json(new RestResult("error", "failed to change password", null, err.toString()));
      });
    }
  } else {
...
exports.delete = function (req, res) {
  User.delete(req.params.id).then(() => {
    ...
    res.json(new RestResult("error", "failed to delete user", null, err.toString()));
  });
};