...
Repository.create = function (record) {
  ...
  return mysql.do("INSERT INTO AnsibleForms.`repositories` set ?", record).then(res => {
...
Repository.update = function (record, name) {
  ...
  return mysql.do("UPDATE AnsibleForms.`repositories` set ? WHERE name=?", [record, name]).then(res => {
