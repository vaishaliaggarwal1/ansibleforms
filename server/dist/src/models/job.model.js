...
var Job = function (job) {
  ...
  if (job.step && job.step != "") {
    // allow single step update
    this.step = job.step;
  }
  if (job.status && job.status != "") {
    // allow single status update
...
Job.printJobOutput = async (data, type, jobid, counter, incrementIssue) => {
  if (incrementIssue) {
    await Job.deleteOutput({
      job_id: jobid
    });
  }
  return await Job.createOutput({
    output: data,
    output_type: type,
    job_id: jobid,
    order: counter
...
Job.findById = async function (user, id, asText, logSafe = false) {
  ...
  try {
    ...
    return [{
      ...job,
      ...{
        output: Helpers.formatOutput(res, asText)
      }
    }];
  } catch (err) {
...
Job.launch = async function (form, formObj, user, creds, extravars, parentId = null, next) {
  ...
  var jobid;
  try {
    jobid = await Job.create(new Job({
      form: form,
      target: target,
      user: user.username,
      user_type: user.type,
  ...
  if (jobtype == "ansible") {
    return await Ansible.launch(extravars, credentials, jobid, null, parentId ? null : formObj.approval // if multistep: no individual approvals checks
    );
  }
  if (jobtype == "awx") {
    return await Awx.launch(extravars, credentials, jobid, null, parentId ? null : formObj.approval,
    // if multistep: no individual approvals checks,
    null, notifications);
  }
  if (jobtype == "multistep") {
    return await Multistep.launch(form, formObj.steps, user, extravars, creds, jobid, null, formObj.approval);
  }
};
Job.continue = async function (form, user, creds, extravars, jobid, next) {
  ...
  if (res.length > 0) {
    ...
    res = await Job.update({
      status: "running"
    }, jobid);
    next({
      id: jobid
    }); // continue result for feedback
    ...
    if (jobtype == "ansible") {
      return await Ansible.launch(extravars, credentials, jobid, ++counter, formObj.approval, true);
    }
    if (jobtype == "awx") {
      return await Awx.launch(extravars, credentials, jobid, ++counter, formObj.approval, true);
    }
    if (jobtype == "multistep") {
...
Job.sendApprovalNotification = async function (approval, extravars, jobid) {
  ...
  try {
    ...
    message = message.replace("${message}", approvalMessage).replaceAll("${url}", url).replaceAll("${baseurl", appConfig.baseUrl).replaceAll("${jobid}", jobid).replaceAll("${title}", subject).replaceAll("${logo}", logo).replaceAll("${color}", color).replaceAll("${color2}", color2);
...
Job.sendStatusNotification = async function (jobid) {
  try {
    ...
    if (!notifications.recipients) {
      ...
      var color2 = "#ffa73b";
      var logo = `${url}${appConfig.baseUrl}assets/img/logo.png`;
      message = message.replace("${message}", job.output.replaceAll("\r\n", "<br>")).replaceAll("${url}", url).replaceAll("${baseurl}", appConfig.baseUrl).replaceAll("${jobid}", jobid).replaceAll("${title}", subject).replaceAll("${logo}", logo).replaceAll("${color}", color).replaceAll("${color2}", color2);
      ...
      try {
        ...
        if (step.approval) {
          if (!approved) {
            ...
            try {
              await Job.update({
                status: "approve",
                approval: JSON.stringify(step.approval),
                end: moment(Date.now()).format('YYYY-MM-DD HH:mm:ss')
              }, jobid);
            } catch (error) {
              logger.error("Failed to update job", error);
        ...
        if (aborted) {
          ...
          if (finalSuccessStatus || step.always) {
            ...
            // previous was not success or there was fail in other steps
            // skip this step
            skipped++;
            logger.debug("skipping: step " + step.name);
            finalSuccessStatus = false;
        ...
        if (step.continue) {
          ...
          // no continue, we fail the multistep
          finalSuccessStatus = false;
          continue;
        }
      }
...
Awx.launchTemplate = async function (template, ev, invent, tags, limit, check, diff, verbose, credentials, awxCredentials, execenv, instanceGroups, scmBranch, jobid, counter) {
  ...
  var extravars = {
    ...ev
  }; // we make a copy of the main extravars
  // merge credentials now
  extravars = {
    ...extravars,
    ...credentials
...
Awx.findExecutionEnvironmentByName = async function (name) {
  ...
  var execution_environment = axiosResult.data.results.find(function (x, index) {
    return x.name == name;
  });
