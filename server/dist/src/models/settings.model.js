...
Settings.find = function () {
  return mysql.do("SELECT * FROM AnsibleForms.`settings` limit 1;").then(res => {
    if (res.length > 0) {
      try {
...
Settings.findMailSettings = function () {
  return mysql.do("SELECT mail_server,mail_port,mail_secure,mail_username,mail_password,mail_from FROM AnsibleForms.`settings` limit 1;").then(res => {
    if (res.length > 0) {
      try {
        if (res[0].mail_password != "") {
          res[0].mail_password = decrypt(res[0].mail_password);
...
Settings.findUrl = function () {
  return mysql.do("SELECT url FROM AnsibleForms.`settings` limit 1;").then(res => {
    if (res.length > 0) {
      return res[0];
    } else {
      logger.error("No settings record in the database, something is wrong");
...
Settings.findFormsYaml = function () {
  return mysql.do("SELECT forms_yaml FROM AnsibleForms.`settings` limit 1;").then(res => {
    if (res.length > 0) {
      return res[0];
    } else {
      logger.error("No settings record in the database, something is wrong");
...
Settings.mailsend = function (to, subject, message) {
  return Settings.findMailSettings().then(config => {
    return Settings.maildo(config, to, subject, message);
  });
};
Settings.maildo = function (config, to, subject, message) {
  ...
  return transporter.sendMail(mailmessage).then(info => {
    if (info.messageId) {
