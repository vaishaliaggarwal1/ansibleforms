...
OIDC.isEnabled = function () {
  return mysql.do("SELECT enabled,groupfilter, issuer FROM AnsibleForms.`oidc` limit 1;").then(res => {
    if (res.length > 0) {
      return res[0];
    } else {
...
OIDC.find = function () {
  return mysql.do("SELECT * FROM AnsibleForms.`oidc` limit 1;").then(res => {
    if (res.length > 0) {
      try {
        if (res[0].secret_id != "") {
