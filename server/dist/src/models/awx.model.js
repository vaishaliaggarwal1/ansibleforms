...
Awx.update = function (record) {
  ...
  return mysql.do("UPDATE AnsibleForms.`awx` set ?", record).then(res => {
    cache.del("awxConfig");
    return res;
  });
};
Awx.find = function () {
  return mysql.do("SELECT * FROM AnsibleForms.`awx` limit 1;").then(res => {
    if (res.length > 0) {
      try {
...
Awx.check = function (awxConfig) {
  ...
  return axios.get(awxConfig.uri + "/api/v2/job_templates/", axiosConfig).then(axiosresult => {
