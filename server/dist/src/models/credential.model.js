...
var Credential = function (credential) {
  ...
  if (credential.port != undefined) {
    this.port = credential.port;
  }
  if (credential.user != undefined) {
    this.user = credential.user;
  }
  if (credential.db_name != undefined) {
    this.db_name = credential.db_name;
  }
  if (credential.secure != undefined) {
    this.secure = credential.secure ? 1 : 0;
  }
  if (credential.is_database != undefined) {
  ...
  if (credential.db_type != undefined) {
...
Credential.findById = function (id) {
  ...
  return mysql.do("SELECT * FROM AnsibleForms.`credentials` WHERE id=?;", id).then(res => {
    if (res.length > 0) {
      try {
...
Credential.findByName2 = function (name) {
  ...
  return mysql.do("SELECT * FROM AnsibleForms.`credentials` WHERE name=?;", name).then(res => {
    if (res.length > 0) {
      try {
        res[0].password = decrypt(res[0].password);
      } catch (e) {
