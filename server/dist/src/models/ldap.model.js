...
Ldap.find = function () {
  return mysql.do("SELECT * FROM AnsibleForms.`ldap` limit 1;").then(res => {
    if (res.length > 0) {
      try {
...
Ldap.check = function (ldapConfig) {
  return new Promise(async (resolve, reject) => {
    const {
      authenticate
    } = require('../lib/ldap-authentication');
    // auth with admin
      ...
      try {
        ...
        if (err.admin) {
          if (err.admin.lde_message) {
            try {
              em = YAML.stringify(err.admin.lde_message);
            } catch (e) {
              em = err;
            }
          } else if (err.admin.code) {
            try {
              em = YAML.stringify(err.admin);
            } catch (e) {
              em = err;
            }
