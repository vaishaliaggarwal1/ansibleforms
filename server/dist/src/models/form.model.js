    ...
    if (settings.forms_yaml) {
      ...
      try {
        if (appConfig.useYtt) {
          ...
          rawdata = execSync(`ytt -f ${quote([appFormsPath])} -f ${quote([formslibdirpath])}${yttEnvDataOpts}${yttLibDataOpts}`, {
            env: process.env,
            encoding: 'utf-8'
          });
        } else {
        ...
        files.forEach((item, i) => {
          try {
            ...
            if (appConfig.useYtt) {
              ...
              itemRawData = execSync(`ytt -f ${quote([itemFormPath])} -f ${quote([formslibdirpath])}${yttEnvDataOpts}`, {
                env: process.env,
                encoding: 'utf-8'
              });
            } else {
  ...
  try {
    ...
    if (files) {
      ...
      backups = files.map(file => {
        ...
        return {
          'file': file,
          'date': dt.format("YYYY-MM-DD kk:mm:ss")
        };
...
Form.parse = function (data) {
  ...
  try {
    ...
    formsConfig = YAML.parse(data.forms, {
      prettyErrors: true
    });
  } catch (err) {
    logger.error("Error : ", err);
...
Form.restore = function (backupName, backupBeforeRestore) {
  ...
  try {
    ...
    if (!backupBeforeRestore) Form.remove(tmpbackup);
    return true;
  } catch (e) {
