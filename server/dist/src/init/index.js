async function init() {
  ...
  try {
    ...
    if (adminGroup.length == 0) {
      ...
      adminGroup = {};
      adminGroup.id = await Group.create(new Group({
        name: adminGroupName
      }));
    } else {
  ...
  for (let record in records) {
    try {
      ...
      if (result.length == 0) {
        ...
        var sql = `INSERT INTO AnsibleForms.${record}(${keys.join(",")}) VALUES(${values.map(() => {
          return "?";
        }).join(",")})`;
  ...
  mysql.do("SELECT name FROM AnsibleForms.`repositories` WHERE rebase_on_start=1").then(repositories => {
  ...
  // this is hourly, abandon running jobs older than a day.
  setInterval(() => {
    mysql.do("SELECT name,cron FROM AnsibleForms.`repositories` WHERE status<>'running' AND cron<>''", undefined, true).then(repositories => {
      repositories.map(repo => {
        try {
