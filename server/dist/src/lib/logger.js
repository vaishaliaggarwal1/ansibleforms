...
const formatColor = winston.format.combine(winston.format.timestamp({
  format: 'YYYY-MM-DD HH:mm:ss:ms'
}), winston.format.printf(info => `${info.timestamp} ${color[info.level] || ''}${info.level}: ${info.message}\x1b[0m`));
const formatNoColor = winston.format.combine(winston.format.timestamp({
  format: 'YYYY-MM-DD HH:mm:ss:ms'
...
var transports = [transportConsole, transportDailyRotateFileErrors, transportDailyRotateFile];
