...
Keygen.exists = function (path, forceRemove) {
  ...
  return fs.promises.access(path).then(() => {
    ...
    if (forceRemove) {
      ...
      return fs.promises.unlink(path).then(() => {
...
Keygen.readKey = function (path, destroy) {
  return fs.promises.readFile(path, 'utf8').then(key => {
    key = key.toString();
...
Keygen.checkAvailability = function (privateKeyPath, force, publicOnly) {
  ...
  if (publicOnly) {
    ...
    return Keygen.exists(privateKeyPath, force).then(exists => {
      if (!exists) {
...
Keygen.ssh_randomart = function (privateKeyPath) {
  ...
  return fs.promises.access(privateKeyPath).catch(() => {
    throw "No private key found";
  }).then(() => {
    return new Promise((resolve, reject) => {
      var output;
      var keygen = spawn(binPath, ['-lvf', privateKeyPath]);
...
Keygen.ssh_keygen = function (privateKeyPath, opts) {
  return new Promise((resolve, reject) => {
    ...
    var output = "";
    if (opts.publicOnly) {
      keygen = spawn(binPath, ['-f', privateKeyPath, '-yq']);
    } else {
      keygen = spawn(binPath, ['-t', 'rsa', '-b', opts.size, '-C', opts.comment, '-N', opts.password, '-f', privateKeyPath, '-m', opts.format]);
    }
    keygen.stdout.on('data', function (a) {
    ...
    keygen.on('exit', function () {
      ...
      if (read) {
        Keygen.readKey(privateKeyPath, destroy).then(privatekey => {
          Keygen.readKey(publicKeyPath, destroy).then(publickey => {
            resolve({
              key: privatekey,
              pubKey: publickey
            });
...
Keygen.keygen = async function (opts, callback) {
  ...
  if (opts.randomArt) {
    ...
    return Keygen.checkAvailability(opts.path, opts.force, opts.publicOnly).then(exists => {
      if (exists) {
