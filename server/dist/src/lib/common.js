...
Helpers.replacePlaceholders = (msg, extravars) => {
  if (!msg) return "";
  return msg.replace(/\$\(([^\)]+)\)/g, (placeholderWithDelimiters, placeholderWithoutDelimiters) => Helpers.findExtravar(extravars, placeholderWithoutDelimiters) || placeholderWithDelimiters);
};
  ...
  records.forEach(function (el) {
    ...
    lines.forEach((line, i) => {
      ...
      if (el.output_type == "stderr") {
        ...
        if (line.match(/^\[WARNING\].*/g)) {
          ...
          matchfound = true;
          line = "<span class='has-text-warning'>" + line + "</span>";
        } else if (line.match(/^\[ERROR\].*/g)) {
          // errors
          previousformat = "danger";
          matchfound = true;
          line = "<span class='has-text-danger'>" + line + "</span>";
        } else if (line.match(/^([A-Z\s]*)[^\*]*(\*+)$/g)) {
          // task line with **** // mark play / task lines as bold
          previousformat = "";
          matchfound = true;
          if (i > 1) {
        ...
        } else if (line.match(/^(ok): \[([^\]]*)\].*/g)) {
          // mark succes lines
          matchfound = true;
          previousformat = "success";
          line = "<span class='has-text-success'>" + line + "</span>";
        } else if (line.match(/^(changed): \[([^\]]*)\].*/g)) {
          // mark change lines
          previousformat = "warning";
          matchfound = true;
          line = "<span class='has-text-warning'>" + line + "</span>";
        } else if (line.match(/^(skipping): \[([^\]]*)\].*/g)) {
          // mark skip lines
          previousformat = "info";
          matchfound = true;
        ...
        if (line.match('ok=.*failed.*')) {
          ...
          matchfound = true;
          previousformat = "";
          line = line.replace(/(ok=[1-9]+[0-9]*)/g, "<span class='tag is-success'>$1</span>").replace(/(changed=[1-9]+[0-9]*)/g, "<span class='tag is-warning'>$1</span>").replace(/(failed=[1-9]+[0-9]*)/g, "<span class='tag is-warning'>$1</span>").replace(/(unreachable=[1-9]+[0-9]*)/g, "<span class='tag is-warning'>$1</span>").replace(/(skipped=[1-9]+[0-9]*)/g, "<span class='tag is-info'>$1</span>");
        }
        if (filterOutput) {
    ...
    lineoutput.forEach(function (el2, i) {
      if (el2 != "" && !addedTimestamp) {
        // we only add timestamp to first non-empty line
